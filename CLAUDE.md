...
This file is read automatically by Claude Code on every session. It is the single source of truth for **how the specialist agents collaborate** when the user asks to build or extend the app.
...
No agent talks directly to another agent. They collaborate through the **shared documents in `/docs/`**. The orchestrator (the main Claude session that reads this file) hands work between agents in the right order.
