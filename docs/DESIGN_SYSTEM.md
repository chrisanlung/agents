...
## Keuangan — tenant-admin (ADR 0015)

_Owned by `ui-ux-expert`. Implemented by `nextjs-expert`. Backend contract in ADR 0015 §2.11–2.12._

_Scope: tenant-admin portal (port 3002) only. No ops-portal or Flutter surfaces._

_All copy in Indonesian. WCAG 2.2 AA throughout. All currency via `formatRupiah` (Rp X.XXX). Dates via `formatDate` / `relativeTime`. Pagination per ADR 0013 (page size 10)._

---

### KEU-A1 — Nav placement

**New top-level entry "Keuangan"** added to `NAV_ITEMS` in `components/app-header.tsx`.

Position: between `"booking"` (Calendar icon) and `"operasional"` (Stethoscope icon) in the `NAV_ITEMS` array.

```ts
{ key: "keuangan", href: "/keuangan", label: "Keuangan", icon: Wallet }
```

- Icon: `Wallet` (lucide-react). Distinct from `Banknote` (reserved for platform-admin).
- `NavKey` union: extend to include `"keuangan"`.
- `getActiveNavKey` helper: add `if (pathname.startsWith("/keuangan")) return "keuangan"`.
- Active style: inherits the existing ring-1 + bg-white/70 pattern for active state.

**Layout file:** create `app/keuangan/layout.tsx` — mirrors `app/booking/layout.tsx` exactly: fetches `/auth/me`, renders `AppBackground` + `AppHeader activeNav="keuangan"`, wraps `<main className="mx-auto max-w-5xl px-6 py-8">`. No sub-nav pills needed (single route at Phase 6).

**Route tree (Phase 6):**

```
/keuangan          — main Keuangan page (three stacked sections)
```

---

### KEU-A2 — Page layout — `/keuangan`

The page renders three stacked sections separated by `space-y-8`.

**Page header:**

```
<h1 className="text-xl font-semibold text-foreground sm:text-2xl">Keuangan</h1>
<p className="mt-1 text-sm text-muted-foreground">Ringkasan pendapatan dan riwayat transaksi Anda.</p>
```

---

#### KEU-A2.1 — Section 1: "Saldo Anda"

Section heading: `<h2 className="text-base font-semibold text-foreground">Saldo Anda</h2>`

Three `<MetricCard>` components in a row — `grid grid-cols-1 gap-4 sm:grid-cols-3`.

| Card | Data source | Background | Label / subtitle |
|---|---|---|---|
| **Dalam Proses** | `SUM(received_amount_idr)` WHERE `payment_transaction.status = 'paid'` | Default card (`bg-card`) | "Dana sedang diproses oleh sistem pembayaran." |
| **Siap Dicairkan** | `SUM(received_amount_idr)` WHERE `status = 'settled' AND disbursement_id IS NULL` | `bg-primary/5 border-primary/30` (actionable highlight) | "Akan dicairkan pada periode berikutnya (Senin)." |
| **Sudah Dicairkan (30 hari)** | `SUM(net_amount_idr)` FROM `tenant_disbursement` WHERE `status = 'transferred' AND transferred_at >= now()-30d` | Default card | "Total dicairkan dalam 30 hari terakhir." |

**MetricCard anatomy (extends existing `MetricCard` in `booking/reports/page.tsx`):**
...
- `valueClassName` for "Siap Dicairkan": `text-primary` (matches the card tint).
- `valueClassName` for "Sudah Dicairkan": `text-emerald-600`.
- `valueClassName` for "Dalam Proses": `text-foreground`.
- `ariaLabel` on the currency `<p>`: pass a screen-reader string generated by a `formatRupiahAria` helper (e.g. `"Rp 1.500.000"` — the formatted string itself is already readable; no complex word-by-word expansion needed for Phase 6; use `aria-label={formatRupiah(amount)}` to ensure the number format is explicitly labeled).

**Loading state:** three `<Card>` skeletons with a rectangle stub in place of the currency value. `animate-pulse bg-muted rounded` stubs, `h-7 w-32` for the value, `h-3 w-48 mt-1` for the subtitle.

**Zero state (new tenant):** all three cards show `Rp 0` — no special state needed; the metric copy is self-explanatory at zero.

---

#### KEU-A2.2 — Section 2: "Riwayat Pencairan"

Section heading: `<h2 className="text-base font-semibold text-foreground">Riwayat Pencairan</h2>`

Table of `tenant_disbursement` rows, sorted by `created_at DESC` by default.
...
| # | `<TableHead>` | Width | Cell content |
|---|---|---|---|
| 1 | Periode | `min-w-[160px]` | `formatDate(period_start)` – `formatDate(period_end)` |
| 2 | Bruto | `w-[130px]` | `formatRupiah(gross_amount_idr)` — `tabular-nums` |
| 3 | Fee Platform (5%) | `w-[140px]` | `formatRupiah(platform_fee_idr)` — `text-muted-foreground tabular-nums` |
| 4 | Net | `w-[130px]` | `formatRupiah(net_amount_idr)` — `font-medium tabular-nums` |
| 5 | Status | `w-[130px]` | `<DisbursementStatusBadge status={...} />` |
| 6 | Tanggal Transfer | `w-[130px]` | `formatDate(transferred_at)` or `—` if null |
| 7 | Ref Bank | `w-[110px]` | `bank_reference` truncated to 12 chars + `…`, full value in `title` attribute. `—` if null. |
| 8 | (action) | `w-[64px]` | `<Button variant="ghost" size="sm">Lihat</Button>` — opens KEU-A6 detail dialog |

`<TableHeader className="bg-muted/30">` — consistent with other list pages.
`<TableRow className="h-14">`, `<TableBody className="text-sm">`.

**Empty state (inside `CardContent`, `py-16`, centered):**

```
<Wallet size={40} aria-hidden="true" className="text-muted-foreground/50 mx-auto" />
<p className="mt-3 text-sm font-medium text-muted-foreground">Belum ada pencairan.</p>
<p className="mt-1 text-xs text-muted-foreground max-w-xs mx-auto">
  Pencairan dilakukan setiap Senin pagi untuk transaksi yang sudah settled.
</p>
```

No CTA needed — tenant cannot trigger disbursement; that is platform-admin only.

**Pagination:** `<Pagination>` component, `PAGE_SIZE = 10`, `pathname="/keuangan"`, preserves query params.

---

#### KEU-A2.3 — Section 3: "Riwayat Transaksi"

Section heading + filter bar in the same row on `sm+`:
...
**Filter bar (inline, not `<FilterBar>` wrapper — simpler at 2 controls):**

```
<div className="flex items-center gap-2 flex-wrap">
  <DateRangePicker fromParam="from" toParam="to" fromLabel="Dari" toLabel="Sampai" />
  <FilterSelect label="Status" name="tx_status"
    options={["", "paid", "settled", "disbursed", "failed", "expired", "voided"]}
    labels={["Semua", "Dibayar", "Settled", "Siap Dicairkan", "Gagal", "Kadaluarsa", "Dibatalkan"]} />
</div>
```

Table of `payment_transaction` rows, sorted by `created_at DESC` by default.
...
| # | `<TableHead>` | Width | Cell content |
|---|---|---|---|
| 1 | Tanggal | `w-[110px]` | `formatDate(created_at)` — `tabular-nums text-muted-foreground` |
| 2 | Booking | `w-[100px]` | `<Link href="/booking/[booking_id]" className="text-primary underline-offset-2 hover:underline font-mono text-xs">{booking_code}</Link>` |
| 3 | Pelanggan | `min-w-[130px]` | `customer_name` from booking |
| 4 | Metode | `w-[90px]` | Provider display label (see map below) |
| 5 | Bruto | `w-[120px]` | `formatRupiah(received_amount_idr ?? expected_amount_idr)` — `tabular-nums`. Show expected for `awaiting`; received for all others. |
| 6 | Fee Platform | `w-[120px]` | `formatRupiah(platform_fee_idr)` or `—` if null — `text-muted-foreground tabular-nums` |
| 7 | Net | `w-[120px]` | `formatRupiah(tenant_net_idr)` or `—` if null — `font-medium tabular-nums` |
| 8 | Status | `w-[130px]` | `<PaymentStatusBadge status={...} />` |

`<TableHeader className="bg-muted/30">`, `<TableRow className="h-14">`, `<TableBody className="text-sm">`.
...
```
<Wallet size={40} aria-hidden="true" className="text-muted-foreground/50 mx-auto" />
<p className="mt-3 text-sm font-medium text-muted-foreground">Belum ada transaksi.</p>
<p className="mt-1 text-xs text-muted-foreground/80 max-w-xs mx-auto">
  Pendapatan dari booking yang sudah dibayar akan tampil di sini.
</p>
```

**Pagination:** `<Pagination>` component, `PAGE_SIZE = 10`, `pathname="/keuangan"`, preserves `from`, `to`, `tx_status` params.

---

### KEU-A3 — PaymentStatusBadge component

New component `components/payment-status-badge.tsx` (mirrors existing `booking-status-badge.tsx` pattern).

```ts
type PaymentStatus = "awaiting" | "paid" | "settled" | "disbursed" | "failed" | "expired" | "voided"
```

| Status | Label | Badge className |
|---|---|---|
| `awaiting` | Menunggu Pembayaran | `border-border text-muted-foreground bg-muted` |
| `paid` | Dibayar — Diproses | `border-transparent bg-primary text-primary-foreground` |
| `settled` | Siap Dicairkan | `border-emerald-300 text-emerald-700 bg-emerald-50` |
| `disbursed` | Sudah Dicairkan | `border-border text-muted-foreground/70 bg-muted` |
| `failed` | Gagal | `border-transparent bg-destructive text-destructive-foreground` |
| `expired` | Kadaluarsa | `border-border text-muted-foreground/60 bg-muted` |
| `voided` | Dibatalkan | `border-transparent bg-destructive/80 text-destructive-foreground` |

All variants use `variant="outline"` on the base `<Badge>` with `className` override — same pattern as `booking-status-badge.tsx`. `whitespace-nowrap` on all.

**Accessibility:** the status text label is the primary semantic signal; color is a reinforcing secondary signal (WCAG 1.4.1).

---

### KEU-A4 — DisbursementStatusBadge component

New component `components/disbursement-status-badge.tsx`.

```ts
type DisbursementStatus = "pending" | "processing" | "transferred" | "failed" | "cancelled"
```

| Status | Label | Badge className |
|---|---|---|
| `pending` | Menunggu | `border-border text-muted-foreground bg-muted` |
| `processing` | Sedang Diproses | `border-amber-300 text-amber-700 bg-amber-50` |
| `transferred` | Sudah Ditransfer | `border-emerald-300 text-emerald-700 bg-emerald-50` |
| `failed` | Gagal | `border-transparent bg-destructive text-destructive-foreground` |
| `cancelled` | Dibatalkan | `border-border text-muted-foreground/60 bg-muted` |

Same `variant="outline"` + `className` override pattern.

---

### KEU-A5 — Empty state (new tenant, zero transactions)

Shown when the page loads and all three balance figures are `Rp 0` AND the transaction table returns an empty array. Replace the full three-section layout with a single centered empty state card.
...
<Card className="py-20">
  <CardContent className="flex flex-col items-center text-center">
    ...
    <h2 className="text-base font-semibold text-foreground">Belum ada transaksi</h2>
    <p className="mt-2 text-sm text-muted-foreground max-w-xs">
      Pendapatan dari booking yang dibayar akan tampil di sini.
    </p>
    <Button variant="link" asChild className="mt-4 text-sm">
      <Link href="/keuangan/bantuan">Pelajari cara menerima pembayaran</Link>
    </Button>
  </CardContent>
</Card>
```

The help link in Phase 6 can route to the KEU-A8 FAQ modal (triggered via a search param e.g. `?faq=1`) rather than a separate route.

---

### KEU-A6 — Disbursement detail dialog

Triggered by clicking "Lihat" on any row in the Riwayat Pencairan table.

**Component:** `DisbursementDetailDialog` — a shadcn `<Dialog>` with `<DialogContent className="max-w-2xl">`.
...
<DialogHeader>
  ...
  <DialogDescription>
    Periode {formatDate(period_start)} – {formatDate(period_end)}
  </DialogDescription>
</DialogHeader>
```
...
1. **Summary row** (`grid grid-cols-3 gap-4 mb-4`): three metric chips — Bruto / Fee 5% / Net — same visual weight as the balance cards but smaller (`text-lg font-bold tabular-nums`).

2. **Transfer details** (`<dl className="text-sm space-y-2 mb-4">`):
   - Status: `<DisbursementStatusBadge>`
   - Tanggal Transfer: `formatDate(transferred_at)` or `—`
   - Ditransfer oleh: admin user name or `—`
   - Ref Bank: `bank_reference` or `—`
   - Catatan: `notes` or `—`

3. **Transaction breakdown** table (scrollable, `max-h-64 overflow-y-auto`):
   - `<Table>` with columns: Tanggal | Booking | Bruto | Fee | Net
   - Each row: one contributing `payment_transaction`
...
Read-only — no edit or action buttons for tenant.

**Accessibility:** `<Dialog>` traps focus; Escape closes; focus returns to the "Lihat" trigger on close (Radix default). `aria-label` on summary metric values.

---

### KEU-A7 — Currency accessibility

For currency `<p>` elements in the balance cards: use `aria-label={formatRupiah(amount)}` where `formatRupiah` produces `"Rp 1.500.000"` — the formatted string is already unambiguous for screen readers. Full word-by-word amount expansion (`"satu juta lima ratus ribu rupiah"`) is deferred; the formatted string satisfies minimum accessibility at Phase 6 scale.

`tabular-nums` class on all currency cells to prevent column width jitter as values update.

---

### KEU-A8 — Help / FAQ modal ("Pelajari Skema Pembayaran")

**Trigger:** small text link at the very bottom of the `/keuangan` page (below Riwayat Transaksi pagination):
...
`setFaqOpen` is a `useState` bool local to the page.
  ...
  <ol className="space-y-4 text-sm">
    <li>
      ...
      <p className="text-muted-foreground mt-1">
        Setelah pelanggan membayar via QRIS, dana masuk ke rekening escrow iPaymu.
        Status transaksi menjadi "Dibayar — Diproses".
      </p>
    </li>
    <li>
      <strong>2. Siap Dicairkan (H+1)</strong>
      <p className="text-muted-foreground mt-1">
        iPaymu mentransfer dana ke rekening Lustia setiap hari kerja (H+1–H+3).
        Status berubah menjadi "Siap Dicairkan". Fee platform 5% dipotong dari total bruto.
      </p>
    </li>
    <li>
      ...
      <p className="text-muted-foreground mt-1">
        ...
        Status berubah menjadi "Sudah Ditransfer".
      </p>
    </li>
  </ol>
  <p className="mt-4 text-xs text-muted-foreground">
    Pertanyaan? Hubungi <a href="mailto:support@lustia.id"
    className="underline">support@lustia.id</a>.
  </p>
</DialogContent>
<DialogFooter>
  <Button variant="outline" onClick={onClose}>Tutup</Button>
...
**Accessibility:** `<Dialog>` focus trap; Escape closes; focus returns to trigger. Copy is plain text — no animation, no interaction beyond dismiss.
...
|---|---|---|
| Balance cards | API fetch fail | Replace the three cards with a single `role="alert"` inline banner: `border-red-300 bg-red-50 text-red-800` + "Gagal memuat saldo. Muat ulang halaman." |
| Disbursement table | API fetch fail | Inline alert inside the `<Card>`: same red banner. |
| Transaction table | API fetch fail | Inline alert inside the `<Card>`. |
| Disbursement detail dialog | API fetch fail | Inline red banner inside the dialog body + `<Button variant="outline">Coba Lagi</Button>`. |
...
| Screen | `<h1>` | Subtitle |
|---|---|---|
| `/keuangan` | Keuangan | Ringkasan pendapatan dan riwayat transaksi Anda. |

**Section headings:**

| Section | Copy |
|---|---|
| Balance cards | Saldo Anda |
| Disbursement history | Riwayat Pencairan |
...
| Event | Message | Variant |
|---|---|---|
| Page load failure (balance) | (inline, not toast — see KEU-A9) | — |
| Disbursement detail load fail | "Gagal memuat detail pencairan. Coba lagi." | destructive |

---

### KEU-A11 — Component reuse inventory

No new shadcn primitives.

| Component | Where |
|---|---|
| `Card`, `CardContent` | Balance cards, table wrappers, empty states |
| `Table`, `TableHeader`, `TableBody`, `TableRow`, `TableHead`, `TableCell` | Both history tables, dialog transaction breakdown |
| `Badge` | All status badges |
| `Button` | "Lihat" row action, dialog close, FAQ link |
| `Dialog`, `DialogHeader`, `DialogTitle`, `DialogDescription`, `DialogContent`, `DialogFooter` | KEU-A6 detail dialog, KEU-A8 FAQ modal |
| `Pagination` | Both history tables |
| `DateRangePicker` | Transaction filter |
| `FilterSelect` | Status filter |

**New components introduced (tenant-admin):**

| Component | File | Description |
|---|---|---|
| `PaymentStatusBadge` | `components/payment-status-badge.tsx` | Maps `PaymentStatus` to labeled colored badge |
| `DisbursementStatusBadge` | `components/disbursement-status-badge.tsx` | Maps `DisbursementStatus` to labeled colored badge |
| `DisbursementDetailDialog` | `app/keuangan/disbursement-detail-dialog.tsx` | Dialog showing full disbursement breakdown |
| `PaymentFaqDialog` | `app/keuangan/payment-faq-dialog.tsx` | FAQ modal explaining 3-tier lifecycle |

**Lucide icons required:**

| Icon | Where |
|---|---|
| `Wallet` | Nav item, balance empty state, zero-state card |
| `AlertCircle` | Inline error alerts |

---

## Settlement & Payout — platform-admin (ADR 0015)

_Owned by `ui-ux-expert`. Implemented by `nextjs-expert`. Backend contract in ADR 0015 §2.9, §2.6, §2.12._

_Scope: platform-admin portal (port 3001) only._

_All copy in Indonesian. WCAG 2.2 AA. All currency via `formatRupiah`. Dates via `formatDate` / `relativeTime`. Page size 10._

---

### POU-A1 — Nav placement (platform-admin)

The platform-admin nav is driven by `components/nav-links.tsx` (client component with `usePathname`). Extend the `links` array:

```ts
const links = [
  { href: "/dashboard",  label: "Dasbor",   icon: LayoutDashboard },
  { href: "/tenants",    label: "Tenant",    icon: Building2 },
  { href: "/payout",     label: "Payout",    icon: Banknote },          // NEW
  { href: "/tenants/registrations", label: "Registrasi", icon: ClipboardList },
] as const;
```

Position: between "Tenant" and "Registrasi". Icon: `Banknote` (lucide-react). Active match: `pathname.startsWith("/payout")`.

**Sub-navigation:** the three Payout pages share a sub-nav identical in pattern to the booking layout sub-nav (pill links below the header). Implement in `app/payout/layout.tsx`:
...
Layout file `app/payout/layout.tsx`: wraps children with header + `<nav aria-label="Navigasi payout" className="mb-6 flex gap-1 border-b border-border pb-3">` sub-nav pills. Active pill: `bg-accent text-accent-foreground rounded-md`; inactive: `text-muted-foreground hover:bg-muted hover:text-foreground rounded-md`. Use a client component for active detection (same `usePathname` pattern as `NavLinks`).

**Route tree:**

```
/payout/reconciliation   — daily reconciliation with iPaymu
/payout/tenant-payout    — per-tenant payout dashboard
/payout/disbursements    — admin work queue
/payout/disbursements/[id] — disbursement detail + admin actions
```

---

### POU-A2 — Reconciliation page — `/payout/reconciliation`

**Purpose:** platform-admin triggers the iPaymu daily settlement report pull; reviews past batches.
...
<div className="flex items-center justify-between">
  <div>
    <h1 className="text-xl font-semibold text-foreground sm:text-2xl">Rekonsiliasi Settlement</h1>
...
```tsx
<Button
  disabled={alreadyReconciledToday}
...
- `alreadyReconciledToday`: true if `settlement_batch` already has a row with `settled_at::date = today`. Computed server-side and passed as a prop.
- When `alreadyReconciledToday = true`: button is `disabled`; add a `<p className="mt-1 text-xs text-muted-foreground">` showing last reconciliation timestamp: `"Terakhir: {relativeTime(lastBatchAt)}"`.
- Loading state while API call in flight: button shows `<Loader2 className="animate-spin" />` + "Menarik laporan..." text. Disable all other page interactions.
...
| # | `<TableHead>` | Width | Cell content |
|---|---|---|---|
| 1 | Tanggal Settle | `w-[130px]` | `formatDate(settled_at)` |
| 2 | Provider | `w-[100px]` | Provider label (iPaymu QRIS / Simulasi) |
| 3 | Transaksi | `w-[90px]`, center | `transaction_count` — `tabular-nums` |
| 4 | Total | `w-[130px]` | `formatRupiah(total_amount_idr)` — `tabular-nums font-medium` |
| 5 | Ditarik | `w-[130px]` | `relativeTime(created_at)` with full date in `title` tooltip |
| 6 | Admin | `min-w-[120px]` | `created_by` user full_name |
| 7 | Aksi | `w-[80px]` | `<Button variant="ghost" size="sm">Detail</Button>` — opens POU-A2 detail modal |

`<TableHeader className="bg-muted/30">`, `<TableRow className="h-14">`, `<TableBody className="text-sm">`.

**Settlement batch detail modal** (triggered by "Detail" button):

```
<Dialog>
  <DialogContent className="max-w-3xl">
    <DialogHeader>
      <DialogTitle>Detail Batch {formatDate(settled_at)}</DialogTitle>
    </DialogHeader>
    [Mismatch warning banner — see POU-A6, shown when mismatches > 0]
    [Transaction table: columns = Tanggal | Provider Reference (mono truncated) | Tenant | Bruto | Status]
    <DialogFooter>
...
<div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
  <RefreshCw size={36} className="text-muted-foreground/40" aria-hidden="true" />
  <p className="text-sm text-muted-foreground">Belum ada rekonsiliasi.</p>
  <p className="text-xs text-muted-foreground">
...
**Pagination:** `<Pagination>` component, `PAGE_SIZE = 10`.

---

### POU-A3 — Tenant payout page — `/payout/tenant-payout`

**Purpose:** per-tenant view of settled-but-not-yet-disbursed amounts; admin initiates a disbursement.
...
<div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
  <div>
    <h1 className="text-xl font-semibold text-foreground sm:text-2xl">Pencairan Tenant</h1>
    <p className="mt-1 text-sm text-muted-foreground">
      Buat pencairan untuk tenant yang memiliki saldo siap dicairkan.
    </p>
  </div>
  [Period selector — see below]
</div>
```

**Period selector:**

A row of two `<DatePicker>` inputs (or `<DateRangePicker>`) defaulting to the current Senin–Minggu period. Stored in URL params `period_start` / `period_end`.

```
<div className="flex items-center gap-2 text-sm">
  <span className="text-muted-foreground">Periode:</span>
  <DateRangePicker fromParam="period_start" toParam="period_end" />
</div>
```

**Tenant card grid:** `grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3`

**TenantPayoutCard** (new component `app/payout/tenant-payout/tenant-payout-card.tsx`):
  ...
  <CardContent className="pt-4">
    <div className="flex items-start justify-between">
      <div>
        ...
        <p className="text-xs text-muted-foreground mt-0.5">{branch_count} cabang</p>
      </div>
    ...
    <div className="mt-4">
      ...
      <p className="mt-1 text-2xl font-bold tabular-nums text-primary"
         aria-label={formatRupiah(settledAmount)}>
        {formatRupiah(settledAmount)}
      </p>
      <p className="mt-0.5 text-xs text-muted-foreground">{txCount} transaksi</p>
    </div>
    <div className="mt-4">
      <Button
        size="sm"
        disabled={settledAmount === 0}
        onClick={() => openCreateModal(tenant.id)}
        className="w-full"
      >
        Buat Pencairan
      </Button>
...
When `settledAmount === 0`: Button is disabled; card shows `text-muted-foreground` for the amount; subtle `bg-muted/30` card background.

**"Buat Pencairan" modal** (opened by card CTA):

`<Dialog>` with `<DialogContent className="max-w-lg">`.

```
<DialogHeader>
  <DialogTitle>Buat Pencairan — {tenant.name}</DialogTitle>
  <DialogDescription>Tinjau kalkulasi sebelum membuat pencairan.</DialogDescription>
</DialogHeader>
...
<div className="space-y-4 py-4">
  <dl className="text-sm space-y-2">
    <div className="flex justify-between">
      ...
      <dd>{formatDate(period_start)} – {formatDate(period_end)}</dd>
    </div>
    <div className="flex justify-between">
      <dt className="text-muted-foreground">Jumlah Transaksi</dt>
      <dd className="tabular-nums">{txCount}</dd>
    </div>
    <div className="flex justify-between">
      <dt className="text-muted-foreground">Bruto</dt>
      <dd className="tabular-nums font-medium">{formatRupiah(gross)}</dd>
    </div>
    ...
    <div className="flex justify-between border-t pt-2">
      ...
      <dd className="tabular-nums font-bold text-primary">{formatRupiah(net)}</dd>
    </div>
  </dl>

  [Scrollable transaction list — max-h-48 overflow-y-auto — Table with Booking | Bruto | Fee | Net]
</div>
...
On successful `POST /admin/disbursements`: close modal, navigate to `/payout/disbursements/[newId]`. Toast: "Pencairan berhasil dibuat. Lakukan transfer manual dan tandai selesai."

**Empty state (no tenants with settled balance):**

```
<div className="flex flex-col items-center justify-center gap-3 py-24 text-center">
  <Banknote size={40} className="text-muted-foreground/40" aria-hidden="true" />
  <p className="text-sm font-medium text-muted-foreground">Tidak ada saldo yang siap dicairkan.</p>
...
### POU-A4 — Active disbursements page — `/payout/disbursements`

**Purpose:** admin work queue — view in-flight disbursements and take action.

**Page header:**

```
<h1 className="text-xl font-semibold text-foreground sm:text-2xl">Disburse Aktif</h1>
<p className="mt-1 text-sm text-muted-foreground">
...
```
<FilterBar isActive={!!statusFilter} resetHref="/payout/disbursements">
  <FilterSelect label="Status" name="status"
    options={["", "pending", "processing", "transferred", "failed"]}
    labels={["Semua", "Menunggu", "Sedang Diproses", "Sudah Ditransfer", "Gagal"]} />
</FilterBar>
```

Default filter: `status=pending,processing` (show only active items by default; admin rarely needs to scroll through `transferred` history on this view). When status param is blank show all.
...
| # | `<TableHead>` | Width | Cell content |
|---|---|---|---|
| 1 | Tenant | `min-w-[150px]` | `tenant.name` — `font-medium` |
| 2 | Periode | `min-w-[160px]` | `formatDate(period_start)` – `formatDate(period_end)` |
| 3 | Net | `w-[130px]` | `formatRupiah(net_amount_idr)` — `tabular-nums font-medium` |
| 4 | Status | `w-[140px]` | `<DisbursementStatusBadge status={...} />` |
| 5 | Dibuat | `w-[120px]` | `relativeTime(created_at)` |
| 6 | Aksi | `w-[90px]` | `<Button variant="outline" size="sm" asChild><Link href="/payout/disbursements/[id]">Kelola</Link></Button>` |

`<TableHeader className="bg-muted/30">`, `<TableRow className="h-14">`, `<TableBody className="text-sm">`.

Row click anywhere navigates to disbursement detail (in addition to the "Kelola" button).

**Empty state:**

```
<div className="flex flex-col items-center justify-center gap-3 py-16 text-center">
  <Banknote size={36} className="text-muted-foreground/40" aria-hidden="true" />
  <p className="text-sm text-muted-foreground">Tidak ada pencairan aktif.</p>
  <p className="text-xs text-muted-foreground">
...
### POU-A5 — Disbursement detail page — `/payout/disbursements/[id]`

**Purpose:** admin reviews full breakdown and drives the disbursement through its status machine.

**Page header:**

```
[Back link: ← Kembali ke Disburse Aktif]
<div className="flex items-start justify-between mt-4">
  <div>
    <h1 className="text-xl font-semibold">Pencairan — {tenant.name}</h1>
    <p className="text-sm text-muted-foreground mt-0.5">
      Periode {formatDate(period_start)} – {formatDate(period_end)}
    </p>
  </div>
  <DisbursementStatusBadge status={disbursement.status} className="text-sm" />
</div>
```

**Layout:** two-column on `lg+`, stacked on mobile. Left column: detail + timeline. Right column: action buttons.
...
<Card>
  <CardContent className="pt-4 space-y-4">
    <dl className="text-sm space-y-2">
      <Row label="Bruto"      value={formatRupiah(gross_amount_idr)} />
      <Row label="Fee Platform 5%" value={`− ${formatRupiah(platform_fee_idr)}`}
           valueClass="text-destructive" />
      <Row label="Net"        value={formatRupiah(net_amount_idr)}
           valueClass="font-bold text-primary" separator />
      <Row label="Ref Bank"   value={bank_reference ?? "—"} />
      <Row label="Catatan"    value={notes ?? "—"} />
      <Row label="Ditransfer oleh" value={transferred_by_name ?? "—"} />
      <Row label="Tanggal Transfer" value={transferred_at ? formatDate(transferred_at) : "—"} />
    </dl>

    [Transaction breakdown table — same as KEU-A6 dialog table]
  </CardContent>
</Card>
```

**Left — Status timeline (POU-A5 visualization):**

Vertical timeline component below the detail card.
    ...
    <ol className="relative border-l border-border space-y-6 ml-3">
      {TIMELINE_STEPS.map(step => (
        <TimelineStep
          key={step.status}
          label={step.label}
          isCompleted={isStepCompleted(disbursement.status, step.status)}
          isActive={disbursement.status === step.status}
          adminName={step.adminName}
...
`TIMELINE_STEPS` order: `pending` → `processing` → `transferred`.

**`TimelineStep` anatomy:**
...
<li className="relative pl-6">
  ...
  {(isCompleted || isActive) && (
    <p className="text-xs text-muted-foreground">
      {adminName} · {relativeTime(timestamp)}
    </p>
  )}
</li>
```

`failed` and `cancelled` statuses appear as a separate `<AlertBanner>` below the timeline, not as a timeline step (they are terminal off-path states, not part of the linear flow).
...
<Card className="sticky top-4">
  <CardContent className="pt-4 space-y-2">
    ...
    {disbursement.status === "pending" && (
      <Button className="w-full" onClick={handleStartProcessing}>Mulai Proses</Button>
    )}
    {disbursement.status === "processing" && (
      <Button className="w-full" onClick={() => setTransferDialogOpen(true)}>
        Tandai Sudah Transfer
      </Button>
    )}
    {["pending", "processing"].includes(disbursement.status) && (
      <>
        <Button variant="outline" className="w-full" onClick={() => setFailDialogOpen(true)}>
          Tandai Gagal
        </Button>
        {disbursement.status === "pending" && (
    ...
    {["transferred", "failed", "cancelled"].includes(disbursement.status) && (
...
Button state rules:
- "Mulai Proses": only when `status === "pending"`. Sends `PATCH /admin/disbursements/:id/status` `{status: "processing"}`.
- "Tandai Sudah Transfer": only when `status === "processing"`. Opens POU-A8 confirmation dialog with bank reference input.
- "Tandai Gagal": when `status` in `["pending", "processing"]`. Opens POU-A8 confirmation dialog with reason.
- "Batalkan": only when `status === "pending"`. Opens POU-A8 confirmation dialog.

All buttons are `disabled` + show `<Loader2>` while the mutation is in flight.

---

### POU-A6 — Reconciliation mismatch warnings

When `settlement_batch` has mismatches (iPaymu reports transactions Lustia does not have, or vice versa), show at the top of the batch detail modal (POU-A2):
...
<div role="alert"
  ...
  <div>
    <p className="font-medium">{mismatch_count} transaksi tidak cocok — investigasi manual diperlukan.</p>
    <button
        ...
        <TableBody>
          {mismatches.map(m => (
            <TableRow key={m.provider_reference}>
              <TableCell className="font-mono text-xs">{m.provider_reference}</TableCell>
              <TableCell className="tabular-nums">{formatRupiah(m.amount_idr)}</TableCell>
              <TableCell>{m.issue_label}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
...
`issue_label` values (display strings, mapped from backend enum):
- `not_in_lustia` → "Ada di iPaymu, tidak ada di Lustia"
- `not_in_provider` → "Ada di Lustia, tidak ada di iPaymu"

---

### POU-A7 — Empty states summary

| Page | Empty condition | Icon | Copy | CTA |
|---|---|---|---|---|
| Reconciliation batch list | No batches ever | `RefreshCw` | "Belum ada rekonsiliasi." + "Tarik laporan settlement iPaymu untuk memulai." | None (CTA is the primary page button at top) |
| Tenant payout | No tenants with settled balance | `Banknote` | "Tidak ada saldo yang siap dicairkan." + supporting line | "Ke Rekonsiliasi" outline button |
| Active disbursements | No rows for filter | `Banknote` | "Tidak ada pencairan aktif." + "Buat pencairan dari halaman Pencairan Tenant." | None |

---

### POU-A8 — Confirmation modals (critical actions)

All three critical actions ("Tandai Sudah Transfer", "Tandai Gagal", "Batalkan") use `<AlertDialog>`.

**"Tandai Sudah Transfer" dialog:**
  ...
  <AlertDialogContent>
    <AlertDialogHeader>
      ...
      <AlertDialogDescription>
        Konfirmasi bahwa transfer manual ke rekening tenant telah dilakukan.
        Tindakan ini akan dicatat di audit log dan tidak dapat dibatalkan.
      </AlertDialogDescription>
    </AlertDialogHeader>
    <div className="space-y-3 py-2">
      <Label htmlFor="bank-ref">Referensi Bank (opsional)</Label>
      <Input id="bank-ref" value={bankRef} onChange={...}
             placeholder="Contoh: TF2026042600001" />
      <Label htmlFor="transfer-notes">Catatan (opsional)</Label>
      <Textarea id="transfer-notes" value={notes} onChange={...} rows={2} />
      <div className="flex items-center gap-2 pt-1">
        <Checkbox id="confirm-audit" checked={confirmed} onCheckedChange={...} />
        <Label htmlFor="confirm-audit" className="text-sm">
    ...
    <AlertDialogFooter>
      ...
      <AlertDialogAction disabled={!confirmed} onClick={handleTransfer}>
...
Same structure. Fields: `reason` (required Textarea, `rows={2}`, placeholder "Jelaskan alasan kegagalan...") + audit-log checkbox. Confirm button: "Ya, Tandai Gagal" (destructive variant). Checkbox `confirmed` AND `reason.length >= 5` must both be true to enable the confirm button.

**"Batalkan" dialog:**

No extra fields (cancellation of a `pending` disbursement that has not been processed is low-risk). Fields: audit-log checkbox only. Confirm: "Ya, Batalkan Pencairan" (destructive). Title: "Batalkan Pencairan?". Body: "Pencairan untuk [tenant.name] akan dibatalkan. Transaksi settled tetap tersimpan dan dapat dicairkan di periode berikutnya."

**Focus rule for all AlertDialogs:** Cancel (`AlertDialogCancel`) is the first focusable element in DOM order inside `AlertDialogFooter` — Radix places focus on the first button, so Cancel receives initial focus. Escape closes without acting.

---

### POU-A9 — Component reuse inventory
...
| `Table`, `TableHeader`, `TableBody`, `TableRow`, `TableHead`, `TableCell` | All table surfaces |
| `Badge` | Status badges |
| `Button` | All CTAs, action buttons |
| `Dialog`, `DialogHeader`, `DialogTitle`, `DialogDescription`, `DialogContent`, `DialogFooter` | Batch detail, create disbursement modal |
| `AlertDialog`, `AlertDialogContent`, `AlertDialogHeader`, `AlertDialogTitle`, `AlertDialogDescription`, `AlertDialogFooter`, `AlertDialogAction`, `AlertDialogCancel` | Confirm modals (POU-A8) |
| `Input` | Bank reference field in transfer dialog |
| `Textarea` | Notes / reason fields in dialogs |
| `Checkbox` | Audit-log confirmation checkbox |
| `Label` | Form labels in dialogs |
| `FilterBar`, `FilterSelect` | Status filter on disbursements page |
| `DateRangePicker` | Period selector on tenant-payout page |
| `Pagination` | All list pages |

**New components introduced (platform-admin):**

| Component | File | Description |
|---|---|---|
| `DisbursementStatusBadge` | `components/disbursement-status-badge.tsx` | Reuse same badge mapping as tenant-admin; copy file or extract to shared lib |
| `TenantPayoutCard` | `app/payout/tenant-payout/tenant-payout-card.tsx` | Per-tenant payout card with CTA |
| `CreateDisbursementDialog` | `app/payout/tenant-payout/create-disbursement-dialog.tsx` | Calculation review + confirm modal |
| `DisbursementTimeline` | `app/payout/disbursements/[id]/disbursement-timeline.tsx` | Vertical status step timeline |
| `TransferConfirmDialog` | `app/payout/disbursements/[id]/transfer-confirm-dialog.tsx` | "Tandai Sudah Transfer" AlertDialog |
| `FailConfirmDialog` | `app/payout/disbursements/[id]/fail-confirm-dialog.tsx` | "Tandai Gagal" AlertDialog |
| `CancelConfirmDialog` | `app/payout/disbursements/[id]/cancel-confirm-dialog.tsx` | "Batalkan" AlertDialog |

**Lucide icons required (new imports in platform-admin):**

| Icon | Where |
|---|---|
| `Banknote` | Nav entry, empty states |
| `RefreshCw` | Reconciliation CTA button, empty state |
| `AlertCircle` | Mismatch warning, error alerts |
| `Loader2` | Loading spinners in buttons |
| `ArrowLeft` | Back link on disbursement detail |

---

### POU-A10 — Accessibility

**Status badges:** all badge text labels are the primary semantic signal; color is reinforcing. No status is communicated by color alone (WCAG 1.4.1).

**Currency values:** `aria-label={formatRupiah(amount)}` on all currency `<p>` elements in metric cards and summary rows.

**Timeline steps:** `<ol>` list with each step as `<li>`. Dot indicator uses `aria-hidden="true"`. The step label + admin name + timestamp are plain text — unambiguous without the visual dot.

**Action buttons on disbursement detail:**

- `aria-label="Mulai proses pencairan untuk [tenant.name]"` on "Mulai Proses" button.
- `aria-label="Tandai sudah transfer untuk [tenant.name]"` on transfer button.
- All buttons `disabled` prop set during loading — maps to `aria-disabled="true"` via shadcn.

**AlertDialog audit-log checkbox:** `<Checkbox id="confirm-audit">` associated with `<Label htmlFor="confirm-audit">`. Confirm button `disabled` until checkbox is checked — prevents accidental critical actions. Focus starts on Cancel (first focusable element in DOM order).

**Keyboard navigation:**

- Modal dialogs: Tab cycles through all focusable elements inside the dialog. Escape closes. Focus returns to trigger on close (Radix default).
- Sub-nav pills: keyboard-navigable `<Link>` elements in document order.

**Reduced motion:** No CSS transitions on timeline dots or card hovers that would need `prefers-reduced-motion` guards. All transitions are `transition-colors` (Tailwind base layer suppresses these automatically under `prefers-reduced-motion: reduce`).
...
| Screen | `<h1>` | Subtitle |
|---|---|---|
| `/payout/reconciliation` | Rekonsiliasi Settlement | Tarik laporan settlement harian dari iPaymu dan cocokkan transaksi. |
| `/payout/tenant-payout` | Pencairan Tenant | Buat pencairan untuk tenant yang memiliki saldo siap dicairkan. |
| `/payout/disbursements` | Disburse Aktif | Kelola pencairan yang sedang berjalan. |
| `/payout/disbursements/[id]` | Pencairan — {tenant.name} | Periode {period_start} – {period_end} |

**CTA buttons:**

| Action | Label | Variant |
|---|---|---|
| Trigger reconciliation | Tarik Laporan iPaymu Hari Ini | primary |
| View batch detail | Detail | ghost size-sm |
| Create disbursement | Buat Pencairan | primary (in card) |
| Confirm create | Buat & Lanjut ke Transfer Manual | primary |
| Start processing | Mulai Proses | primary |
| Mark transferred | Tandai Sudah Transfer | primary |
| Mark failed | Tandai Gagal | outline |
| Cancel disbursement | Batalkan | ghost destructive |
| Confirm transfer | Ya, Tandai Sudah Transfer | AlertDialogAction |
| Confirm fail | Ya, Tandai Gagal | AlertDialogAction destructive |
| Confirm cancel | Ya, Batalkan Pencairan | AlertDialogAction destructive |
| Manage disbursement | Kelola | outline size-sm |

**Toasts:**

| Event | Message | Variant |
|---|---|---|
| Reconciliation triggered | "Laporan iPaymu berhasil ditarik. {N} transaksi disettlement." | success |
| Reconciliation failed | "Gagal menarik laporan iPaymu. Coba lagi." | destructive |
| Disbursement created | "Pencairan berhasil dibuat. Lakukan transfer manual dan tandai selesai." | success |
| Disbursement creation failed | "Gagal membuat pencairan. Coba lagi." | destructive |
| Status moved to processing | "Pencairan dipindahkan ke status 'Sedang Diproses'." | success |
| Status moved to transferred | "Pencairan ditandai sudah ditransfer." | success |
| Status moved to failed | "Pencairan ditandai gagal." | success |
| Status cancelled | "Pencairan dibatalkan." | success |
| Any status mutation failed | "Gagal memperbarui status. Coba lagi." | destructive |

---

### POU-A12 — Open questions for orchestrator

1. **`DisbursementStatusBadge` deduplication.** Both tenant-admin and platform-admin need `DisbursementStatusBadge` with identical logic. They are separate Next.js apps in separate directories. Options: (a) copy the component file into both apps (simple, current pattern for shared primitives); (b) extract to a shared package under `/web/packages/`. Recommend option (a) for Phase 6 since the apps are independently deployable and a shared package adds build complexity. Flag to orchestrator if this decision changes.

2. **`Checkbox` not yet vendored in platform-admin.** `AlertDialog` confirmation dialogs (POU-A8) use `<Checkbox>`. Confirm with `nextjs-expert` whether `components/ui/checkbox.tsx` is vendored in the platform-admin app. If not, `nextjs-expert` runs `npx shadcn add checkbox` before implementing POU-A8.

3. **Transaction list in disbursement detail.** The backend must return the list of `payment_transaction` rows contributing to a `tenant_disbursement` via `GET /admin/disbursements/:id` (or a sub-resource endpoint). Confirm with `go-expert` that the DTO includes `transactions[]` with `{booking_id, booking_code, customer_name, received_amount_idr, platform_fee_idr, tenant_net_idr}`. This data is required for KEU-A6, POU-A3 (creation modal), and the POU-A5 detail page table. **Cross-agent impact: go-expert must include `transactions[]` in the disbursement detail DTO.**

4. **Mismatch data shape.** POU-A6 assumes the `settlement_batch` detail DTO includes a `mismatches[]` array. Confirm with `go-expert` that `GET /admin/settlement-batches/:id` returns `mismatches: [{provider_reference, amount_idr, issue: "not_in_lustia"|"not_in_provider"}]`. **Cross-agent impact: go-expert.**
