...
/**
 * Server Action: login for the ops portal.
 *
 * Decision tree (ADR 0007 §2.4):
 *   scope=tenant + roles includes branch_admin/finance/therapist → set cookies, redirect /dashboard
...
export async function loginAction(
  ...
  if (loginResponse.scope === "platform") {
    return {
      status: "error",
      toast: "Portal ini hanya untuk staf operasional.",
    };
  }
  ...
  // scope=tenant → single membership, check for at least one ops role.
