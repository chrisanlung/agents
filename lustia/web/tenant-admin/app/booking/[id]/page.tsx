...
export default async function BookingDetailPage({ params }: PageProps) {
  ...
  // Determine if caller is branch_admin (read-only mode for cancel)
  ...
  return (
    <div className="space-y-6">
      ...
      {/* branch_admin read-only banner */}
      {isBranchAdmin && (
        <div
          role="note"
          className="rounded-md border border-border bg-muted/40 px-4 py-2 text-sm text-muted-foreground"
        >
          Anda hanya dapat melihat detail booking. Hubungi tenant admin untuk
          membatalkan.
        </div>
      )}
        ...
        <div className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <InfoRow label="WhatsApp" value={booking.customer_phone} />
...
function InfoRow({
  ...
  return (
    <div className="flex items-start justify-between gap-2">
      ...
      <span
        className={`text-right text-sm ${bold ? "font-semibold text-foreground" : "text-foreground"}`}
      >
        {value}
      </span>
    </div>
