...
/**
 * Change password server action for platform-admin.
 *
 * Backend's ChangePassword revokes EVERY refresh token for this user (including
 * the current one), so we cannot rotate the access-token in place. We clear the
 * session cookies and let the form redirect the user to /login to re-authenticate
