...
// Selecting PAYMENT_PROVIDER=dummy in a production build is a hard startup
// failure — not a silent fallback.
func newDummyOrError(env string) (ProviderIface, error) {
	return nil, fmt.Errorf(
		"CRITICAL SECURITY: PAYMENT_PROVIDER=dummy is not available in this binary "+
			"(built without dev/local tag); APP_ENV=%q. "+
			"Rebuild with -tags dev for local development, or set PAYMENT_PROVIDER=ipaymu "+
			"(SECURITY.md C-2)",
		env,
	)
}
